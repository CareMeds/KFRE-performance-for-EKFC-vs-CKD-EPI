...
alb_cys_creat <- merge(alb, cys_creat, by = "lopnr") |>
  ...
  dplyr::mutate(
    diff = abs(as.numeric(as.Date(date_alb) - as.Date(date_creat)))
  ) |>
...
alb_cys_creat_death <- alb_cys_creat |>
  ...
  dplyr::filter(
    is.na(death_date) | date_cys <= death_date & date_alb <= death_date
  )
...
alb_cys_creat <- alb_cys_creat_death |>
  dplyr::mutate(
    date_baseline = as.Date(ifelse(
      as.Date(date_alb) < as.Date(date_creat),
      as.Date(date_creat),
      as.Date(date_alb)
    ))
  )
...
# save final cohort
save(cohort, file = here("Data", "clean", "cohort.Rdata"))
