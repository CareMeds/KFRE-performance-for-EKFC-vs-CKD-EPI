...
library(here) # data manipulation

# Load data sets
load(here("Data", "clean", "cohort.Rdata"))
load(here("Data", "raw", "emigration.Rda")) # emigration
load(here("Data", "raw", "diagnoses_kon.Rda")) # primary care
load(here("Data", "raw", "diagnoses_ovr.Rda")) # specialist care
load(here("Data", "raw", "diagnoses_slv.Rda")) # hospitalizations
load(here("Data", "raw", "medication.Rda")) # medications
...
cohort <- cohort |>
  ...
  dplyr::mutate(
    diff = as.numeric(as.Date(date_emigration) - as.Date(date_baseline)),
...
cohort_egfr <- cohort |>
  dplyr::mutate(
    ...
    ekfc_cys = ekfc_cys(cys, age),
    ekfc_cr_cys = ekfc_cr_cys(creat, cys, age, female)
  )
...
save(diagnoses_index_dt, file = here("Data", "clean", "diagnoses.Rdata"))
...
diagnoses_per_ID <- diagnoses_index_dt |>
  dplyr::filter(
    date_diag <= index_dt & # diagnosis before any time before baseline
      !is.na(date_diag)
  ) |>
...
comorb_index <- data.frame(
  ...
  ICD = c(
    "^I200|^I21|^I22", # myocaridal infarction
    ...
    "^B18|^I982|^K70|^K71|^K72|^K73|^K74|^K75|^K76|^K77"
  )
) # liver disease
for (ICD_index in 1:length(comorb_index$names)) {
  ...
  cohort <- cohort |>
    ...
    dplyr::mutate(
      !!comorb_index$names[ICD_index] := tidyr::replace_na(
        # those who are not in diagnoses_per_ICD did not have that comorbidity
        !!dplyr::sym(comorb_index$names[ICD_index]),
        0
      )
    )
}
...
diagnoses_per_ID_1y <- diagnoses_index_dt |>
  dplyr::filter(
    date_diag <= index_dt & # diagnosis before baseline
...
medication_per_ID <- medication_index_dt |>
  dplyr::filter(
    edatum >= (index_dt - 183) & # only look back half a year
...
# Obtain medications
medications_index <- data.frame(
  names = c("bblock", "hypoglycemic", "ccb", "diur", "rasi", "lipid", "nsaid"),
  ATC = c(
    "^C07", # beta blockers
...
for (ATC_index in 1:length(medications_index$names)) {
  ...
  medication_per_ATC <- medication_per_ID |>
    dplyr::mutate(
      ATC_present = grepl(medications_index$ATC[ATC_index], medication)
    ) |>
    dplyr::group_by(lopnr) |>
  ...
  cohort <- cohort |>
    ...
    dplyr::mutate(
      !!medications_index$names[ATC_index] := tidyr::replace_na(
        # those who are not in diagnoses_per_ICD did not have that comorbidity
        !!dplyr::sym(medications_index$names[ATC_index]),
        0
      )
    )
}
...
save(cohort, file = here("Data", "clean", "cohort_covariates.Rdata"))
